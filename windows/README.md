...
transient treatment for `../_shared/cttc-gateway.tar.gz` -- see "How a
release is cut".
...
2. **Registry pull** (`../_repo/`) -- `image.json` names an image + tag on a
   container registry (e.g. `osteck/cttc-gateway:0.0.1` on Docker Hub) for
...
- builds the server image once via `../_shared/build-image.sh`
  ...
  `../_shared/cttc-gateway.tar.gz` (gitignored -- a build artifact, not
