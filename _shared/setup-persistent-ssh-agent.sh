...
# cttc-server container's SSH_AUTH_SOCK mount (see docker-compose.yml) keeps
...
# cttc-server container -- not on your client machine, and not over the
...
#   - writes/updates cttc-server/.env with SSH_AUTH_SOCK=<that path>, so
...
COMPOSE_DIR="$HOME/cttc-server"   # matches server-provision.js's remoteDir
